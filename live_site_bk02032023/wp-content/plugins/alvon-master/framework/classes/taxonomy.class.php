 ...
 *
 */
class ALVONFramework_Taxonomy extends ALVONFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'alvon_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && ALVON_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = alvon_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $alvon_errors = get_transient( 'alvon-taxonomy-transient' );

    wp_nonce_field( 'alvon-taxonomy', 'alvon-taxonomy-nonce' );

    echo '<div class="alvon-framework alvon-taxonomy alvon-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo alvon_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( alvon_get_var( 'alvon-taxonomy-nonce' ), 'alvon-taxonomy' ) ) {

      $errors = array();
      $taxonomy = alvon_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'alvon_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'alvon_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'alvon_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'alvon_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'alvon_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'alvon-taxonomy-transient', $errors, 10 );
